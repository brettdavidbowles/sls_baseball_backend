...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        games_to_run = Game.objects.filter(
            date_time__hour=current_time.hour, date_time__day=current_time.day)
        for game in games_to_run:
            if game.at_bats.exists():
                continue

            try:
                home_team_lineup = Lineup.objects.select_related('game').get(
                    game=game, team=game.home_team).players.all().order_by('batting_order_number')
            except Lineup.DoesNotExist:
                try:
                    home_team_lineup = Lineup.objects.select_related('team').filter(team=game.home_team).order_by(
                    ...
                    new_home_lineup = Lineup(game=game, team=game.home_team)
                    new_home_lineup.save()
                    new_home_team_lineup_players = Player.objects.select_related(
                        'team').filter(team=game.home_team)[0:10]
            ...
            mapped_home_team_lineup = [
                {**p.player.__dict__, 'attributes': {**p.player.attributes.__dict__}} for p in home_team_lineup]

            try:
                away_team_lineup = Lineup.objects.select_related('game').get(
                    game=game, team=game.away_team).players.all().order_by('batting_order_number')
            except Lineup.DoesNotExist:
                try:
                    away_team_lineup = Lineup.objects.select_related('team').filter(team=game.away_team).order_by(
                        'game__date_time').last().players.all().order_by('batting_order_number')
                    ...
                    new_away_lineup = Lineup(game=game, team=game.away_team)
                    new_away_lineup.save()
                    new_away_team_lineup_players = Player.objects.select_related(
                        'team').filter(team=game.away_team)[0:10]
            ...
            mapped_away_team_lineup = [
                {**p.player.__dict__, 'attributes': {**p.player.attributes.__dict__}} for p in away_team_lineup]
            ...
            new_game = play_ball(
                home_team_batters, home_team_pitcher, away_team_batters, away_team_pitcher)
            ...
            mapped_at_bats = [AtBat(
                game=game,
                pitcher=next(
                    player for player in all_game_players if player.player_id == atbat['pitcher_id']),
                batter=next(
                    player for player in all_game_players if player.player_id == atbat['batter_id']),
                ...
                for index, runner in enumerate(atbat['new_runners_on']):
                    if runner:
                        mapped_runners_left_on.append(LeftOnRunner(
                            at_bat=AtBat.objects.get(
                                game__id=game.id, game_at_bat_number=atbat['game_at_bat_number']),
                            player=next(
                                player for player in all_game_players if player.player_id == runner['id']).player,
                            base=index + 1,
                            at_bat_subindex=0
